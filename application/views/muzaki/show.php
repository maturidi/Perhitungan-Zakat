...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Update Data Muzaki</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <form action="<?php echo base_url(); ?>muzaki/edit/<?php if($row){ echo $row->id_muzaki; }else{ echo $id; }?>" method="POST">
								<div class="form-group">
									<label>Nama Muzaki</label>
									<input type="text" value="<?php if($row){ echo $row->nama_muzaki; }else{ echo set_value('nama'); } ?>" class="form-control" name="nama" placeholder="Nama"></input>
									<?php echo form_error('nama', '<p style="color: #a94442">', '</p>'); ?>
								</div>
								<div class="form-group">
									<label>Alamat Muzaki</label>
									<input type="text" value="<?php if($row){ echo $row->alamat_muzaki; }else{ echo set_value('alamat'); } ?>" class="form-control" name="alamat" placeholder="Alamat"></input>
									<?php echo form_error('alamat', '<p style="color: #a94442">', '</p>'); ?>
								</div>
								<div class="form-group">
									<label>No. Telepon Muzaki</label>
									<input type="text" value="<?php if($row){ echo $row->tlp_muzaki; }else{ echo set_value('tlp'); } ?>" class="form-control" name="tlp" placeholder="No. Telepon"></input>
									<?php echo form_error('tlp', '<p style="color: #a94442">', '</p>'); ?>
								</div>
								<div class="form-group">
									<label>Jenis Kelamin Muzaki</label>
									<select name="jk" class="form-control">
										<?php if ($row->jk_muzaki == 'Laki - Laki' || set_value('jk') == 'Laki - Laki') {
