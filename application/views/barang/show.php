...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Update Data Barang</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <form action="<?php echo base_url(); ?>barang/edit/<?php if($row){ echo $row->id_barang; }else{ echo $id; } ?>" method="POST">
								<div class="col-lg-12">
									<div class="col-lg-8">
									<label>Pilih Muzaki Terlebih Dahulu</label>
										<div class="form-group">
											<input type="text" value="<?php if($this->session->userdata('id_muzaki')){ echo $this->session->userdata('id_muzaki'); }elseif($row){ echo $row->id_muzaki; }else{ echo set_value('id_muzaki'); }?>" class="form-control" name="id_muzaki" placeholder="ID Muzaki" readonly></input>
											<?php echo form_error('id_muzaki', '<p style="color: #a94442">', '</p>'); ?>
										</div>
									</div>
									<div class="col-lg-4" style="padding-top:  25px;">
										<a href="<?php echo base_url(); ?>barang/choose/<?php if($row){ echo $row->id_barang; }else{ echo $id; } ?>" class="btn btn-primary"><span class="glyphicon glyphicon-record"></span> Pilih Muzaki</a>
									</div>
								</div>
								<div class="col-lg-12">
								<div class="col-lg-12">
									<div class="form-group">
										...
										<select name="id_jenis" class="form-control">
											<?php
											foreach($jenis_zakat->result() as $r) {
												if ($row->id_jenis == $r->id_jenis || $r->id_jenis == set_value('id_jenis')) {
													echo '<option value="'. $r->id_jenis .'" selected>'. ucwords($r->nama_jenis) .'</option>';
												} else {
													echo '<option value="'. $r->id_jenis .'">'. ucwords($r->nama_jenis) .'</option>';
												}
											}
											?>
										</select>
										<?php echo form_error('id_jenis', '<p style="color: #a94442">', '</p>'); ?>
									</div>
								</div>
								</div>
								<div class="col-lg-12">
									<div class="col-lg-6">
										<div class="form-group">
											...
											<input type="text" value="<?php if($row){ echo $row->masuk; }else{ echo set_value('masuk'); }?>" class="form-control" name="masuk" placeholder="Barang Masuk"></input>
											<?php echo form_error('masuk', '<p style="color: #a94442">', '</p>'); ?>
										</div>
									</div>
									<div class="col-lg-6">
										<div class="form-group">
											...
											<select name="id_jns_barang" class="form-control">
												<?php
												foreach($jenis_barang->result() as $r) {
													if ($row->id_jns_barang == $r->id_jns_barang || $r->id_jns_barang == set_value('id_jns_barang')) {
														echo '<option value="'. $r->id_jns_barang .'" selected>'. ucwords($r->nama_jns_barang) .' ('. ucwords($r->satuan_jns_barang) .')</option>';
													} else {
														echo '<option value="'. $r->id_jns_barang .'">'. ucwords($r->nama_jns_barang) .' ('. ucwords($r->satuan_jns_barang) .')</option>';
													}	
												}
												?>
											</select>
								...
								<div class="col-lg-12">
									<div class="form-group">
										...
										<input type="text" value="<?php if($row){ echo $row->tgl_masuk; }else{ echo set_value('tgl_masuk'); } ?>" class="form-control datepicker" id="dtp_masuk" name="tgl_masuk" placeholder="Tanggal Masuk"></input>
										<?php echo form_error('tgl_masuk', '<p style="color: #a94442">', '</p>'); ?>
									</div>
								</div>
								</div>
								<input type="submit" class="btn btn-success" value="Save" style="margin-left: 30px;"> 
							</form>
                        </div>
                        <!-- /.panel-body -->
                    </div>
	...
	<script>
		$('#dtp_masuk').datepicker({format:'yyyy-mm-dd'});
	</script>
