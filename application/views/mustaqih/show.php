...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Update Data Mustahiq</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <form action="<?php echo base_url(); ?>mustahiq/edit/<?php if($row){ echo $row->id_mustahiq; }else{ echo $id; } ?>" method="POST">
								<div class="form-group">
									<label>Nama Mustahiq</label>
									<input type="text" value="<?php if($row){ echo $row->nama_mustahiq; }else{ echo set_value('nama'); }?>" class="form-control" name="nama" placeholder="Nama"></input>
									<?php echo form_error('nama', '<p style="color: #a94442">', '</p>'); ?>
								</div>
								<div class="form-group">
									<label>Alamat Mustahiq</label>
									<input type="text" value="<?php if($row){ echo $row->alamat_mustahiq; }else{ echo set_value('alamat'); }?>" class="form-control" name="alamat" placeholder="Alamat"></input>
									<?php echo form_error('alamat', '<p style="color: #a94442">', '</p>'); ?>
								</div>
								<div class="form-group">
									<label>Umur</label>
									<input type="text" value="<?php if($row){ echo $row->umur_mustahiq; }else{ echo set_value('umur'); }?>" class="form-control" name="umur" placeholder="Umur"></input>
									<?php echo form_error('umur', '<p style="color: #a94442">', '</p>'); ?>
								</div>
								<div class="form-group">
									<label>Jenis Kelamin Mustahiq</label>
									<select name="jk" class="form-control">
										<?php if ($row->jk_mustahiq == 'Laki - Laki' || set_value('jk') == 'Laki - Laki') {
											echo '<option value="Laki - Laki" selected>Laki - Laki</option>
												<option value="Perempuan">Perempaun</option>';
										} else {
											echo '<option value="Laki - Laki">Laki - Laki</option>
												<option value="Perempuan" selected>Perempaun</option>';
										}?>
									</select>
									<?php echo form_error('jk', '<p style="color: #a94442">', '</p>'); ?>
								</div>
								<div class="form-group">
									<label>Macam Mustahiq</label>
									<select name="macam" class="form-control">
										<?php
										foreach($macam->result() as $r) {
											if ($row->id_kategori == $r->id_macam || $r->id_macam == set_value('macam')) {
												echo '<option value="'. $r->id_macam .'" selected>'. ucwords($r->nama_macam) .'</option>';
											} else {
												echo '<option value="'. $r->id_macam .'">'. ucwords($r->nama_macam) .'</option>';
											}
										}
										?>
									</select>
									<?php echo form_error('macam', '<p style="color: #a94442">', '</p>'); ?>
